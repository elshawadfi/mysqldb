...
class MysqlDB {
	...
	/**
	 * When you want to insert an expression to the query, to avoid MysqlDB
	 ...
	 * $student = array(
   *    'name'    => "Johny",
   *    'created' => MysqlDB::createExpression('NOW()')
   * );
   * $db->insert("students", $student);
	 * 
	 */
	public static function createExpression($sql_expression) {
	  return new MysqlDBExpressionType($sql_expression);
	}
  ...
  public function query_row($sql, $type = "assoc") {
	...
	private function get_field_value($value) {
	  if ($value instanceof MysqlDBExpressionType) {
